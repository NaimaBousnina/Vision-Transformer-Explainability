...
def get_config(args):
    ...
    if args.workers_id == 'cpu' or not torch.cuda.is_available():  #if no GPU is available
        configuration['GPU_ID'] = []
        print("check", args.workers_id, torch.cuda.is_available())
    else:
        configuration['GPU_ID'] = [int(i) for i in args.workers_id.split(',')]  # When using a single GPU or multiple GPUs
    ...
    assert args.net in [ 'VIT','VITs','VIT_LRP']    
    configuration['BACKBONE_NAME'] = args.net
    assert args.head in ['Softmax', 'ArcFace', 'CosFace', 'SFaceLoss'] #loss functions
    configuration['HEAD_NAME'] = args.head
    configuration['TARGET'] = [i for i in args.target.split(',')] #test
