...
class FaceDataset(data.Dataset):
    def __init__(self, path_imgrec, rand_mirror):
        ...
        if path_imgrec:
            logging.info('loading recordio %s...',
                         path_imgrec)
            path_imgidx = path_imgrec[0:-4] + ".idx"  #need the path name without ".rec" to otain the idx path
            print(path_imgrec, path_imgidx) # i have two paths, the train.rec and train.idx
            self.imgrec = recordio.MXIndexedRecordIO(path_imgidx, path_imgrec, 'r')
            s = self.imgrec.read_idx(0)
            header, _ = recordio.unpack(s)  # Unpack a MXImageRecord to string
            if header.flag > 0:
    ...
    def __getitem__(self, index):
        ...
        return img, label    #now i have access just like a an array class[index]-> image and label
